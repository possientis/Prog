...
Definition recursion (f a:U) : U := RecursionOfClass.recursion (toClass f) a.
...
Proof.
  intros f. apply RecursionOfClass.Charac.
Qed.
...
Proposition Charac2 : forall (f a y z:U),
  ...
  exists g b,
    ...
    (forall c, c :< b -> g!c = f!(g:|:c)).
Proof.
  intros f. apply RecursionOfClass.Charac2.
Qed.
...
Proof.
  intros f. apply RecursionOfClass.IsFunctionOn.
Qed.
...
Proof.
  intros f. apply RecursionOfClass.IsRecursive.
Qed.
...
Proposition IsUnique : forall (f g a:U),
  ...
  g = recursion f a.
Proof.
  intros f g a. apply RecursionOfClass.IsUnique.
Qed.
