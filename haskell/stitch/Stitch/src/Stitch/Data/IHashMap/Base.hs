...
module  Stitch.Data.IHashMap.Base
  ( IHashMap  (..)
  , IHashable (..)
  , Leaf      (..)
...
import qualified Stitch.Data.IHashMap.Array   as A

import Stitch.Data.Exists
import Stitch.Data.IHashable
import Stitch.Data.IHashMap.List 
import Stitch.Data.IHashMap.UnsafeShift
...
eq x y = case testEquality x y of 
  Just _  -> True
  Nothing -> False
infix 4 `eq`
{-# INLINE eq #-}
...
instance (forall i. NFData (k i), forall i. NFData (v i)) => NFData (Leaf k v) 
  where rnf (L k v) = rnf k `seq` rnf v
...
instance (forall i. NFData (k i), forall i. NFData (v i)) => NFData (IHashMap k v)
  where
    rnf Empty                 = ()
        ...
        | otherwise =
            ...
            in if st' `ptrEq` st
                ...
                Empty | A.length ary == 1 -> Empty
                      ...
                      | otherwise -> bIndexed
                    where
                      bIndexed = BitmapIndexed 
                        (b .&. complement m) (A.delete ary i)
