...
module Stitch.Data.IHashMap.Array
  ( Array
  , MArray
...
import GHC.Exts (copySmallArray#, copySmallMutableArray#, indexSmallArray#)
import GHC.Exts (Int(..), Int#, newSmallArray#, readSmallArray#)
import GHC.Exts (reallyUnsafePtrEquality#, sizeofSmallArray#)
import GHC.Exts (sizeofSmallMutableArray#, SmallArray#, SmallMutableArray#)
import GHC.Exts (State#, tagToEnum#, thawSmallArray#, unsafeCoerce#)
import GHC.Exts (unsafeFreezeSmallArray#, unsafeThawSmallArray#, writeSmallArray#)
...
index :: Array a -> Int -> a
index ary _i@(I# i#) = case indexArray# (unArray ary) i# of 
  (# b #) -> b
{-# INLINE index #-}
...
indexM ary _i@(I# i#) = case indexArray# (unArray ary) i# of 
  (# b #) -> return b
{-# INLINE indexM #-}

unsafeFreeze :: MArray s a -> ST s (Array a)
unsafeFreeze mary = ST $ \s -> 
  case unsafeFreezeArray# (unMArray mary) s of
    (# s', ary #) -> (# s', array ary (lengthM mary) #)
{-# INLINE unsafeFreeze #-}

unsafeThaw :: Array a -> ST s (MArray s a)
unsafeThaw ary = ST $ \s -> case unsafeThawArray# (unArray ary) s of
...
copy !src !_sidx@(I# sidx#) !dst !_didx@(I# didx#) _n@(I# n#) = ST $ \ s# ->
  case copyArray# (unArray src) sidx# (unMArray dst) didx# n# s# of
    s2 -> (# s2, () #)

-- | Unsafely copy the elements of an array. Array bounds are not checked.
copyM :: MArray s e -> Int -> MArray s e -> Int -> Int -> ST s ()
copyM !src !_sidx@(I# sidx#) !dst !_didx@(I# didx#) _n@(I# n#) = ST $ \ s# ->
...
traverse f = \ary -> 
  fromList (length ary) `fmap` Traversable.traverse f (toList ary)
{-# INLINE traverse #-}
