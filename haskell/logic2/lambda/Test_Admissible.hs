...
test4  = TestCase  $ assertBool "test4"   $       isAdmissibleFor (y<-:x) p4
...
test13 = TestCase  $ assertBool "test13"  $       isAdmissibleFor (z<-:x) p5
test14 = TestCase  $ assertBool "test14"  $       isAdmissibleFor (z<-:x) p6
test15 = TestCase  $ assertBool "test15"  $       isAdmissibleFor (z<-:x) p7
test16 = TestCase  $ assertBool "test16"  $       isAdmissibleFor (z<-:x) p8
...
test25  = TestCase  $ assertBool "test25" $       isAdmissibleFor (x<-:y) p1
test26  = TestCase  $ assertBool "test26" $ not $ isAdmissibleFor (x<-:y) p2
test27  = TestCase  $ assertBool "test27" $ not $ isAdmissibleFor (x<-:y) p3
test28  = TestCase  $ assertBool "test28" $       isAdmissibleFor (x<-:y) p4
test29  = TestCase  $ assertBool "test29" $ not $ isAdmissibleFor (x<-:y) p5
test30  = TestCase  $ assertBool "test30" $       isAdmissibleFor (x<-:y) p6
...
test41  = TestCase  $ assertBool "test41" $       isAdmissibleFor (z<-:y) p1
test42  = TestCase  $ assertBool "test42" $ not $ isAdmissibleFor (z<-:y) p2
test43  = TestCase  $ assertBool "test43" $ not $ isAdmissibleFor (z<-:y) p3
test44  = TestCase  $ assertBool "test44" $       isAdmissibleFor (z<-:y) p4
test45  = TestCase  $ assertBool "test45" $       isAdmissibleFor (z<-:y) p5
test46  = TestCase  $ assertBool "test46" $       isAdmissibleFor (z<-:y) p6
test47  = TestCase  $ assertBool "test47" $       isAdmissibleFor (z<-:y) p7
test48  = TestCase  $ assertBool "test48" $       isAdmissibleFor (z<-:y) p8
...
test53  = TestCase  $ assertBool "test53" $       isAdmissibleFor (x<-:z) p5
test54  = TestCase  $ assertBool "test54" $       isAdmissibleFor (x<-:z) p6
test55  = TestCase  $ assertBool "test55" $       isAdmissibleFor (x<-:z) p7
test56  = TestCase  $ assertBool "test56" $       isAdmissibleFor (x<-:z) p8
...
test61  = TestCase  $ assertBool "test61" $       isAdmissibleFor (y<-:z) p5
test62  = TestCase  $ assertBool "test62" $       isAdmissibleFor (y<-:z) p6
test63  = TestCase  $ assertBool "test63" $       isAdmissibleFor (y<-:z) p7
test64  = TestCase  $ assertBool "test64" $       isAdmissibleFor (y<-:z) p8
...
test82  = TestCase  $ assertBool "test82" $ not $ isAdmissibleFor (y<->x) p2
test83  = TestCase  $ assertBool "test83" $ not $ isAdmissibleFor (y<->x) p3
test84  = TestCase  $ assertBool "test84" $       isAdmissibleFor (y<->x) p4
test85  = TestCase  $ assertBool "test85" $       isAdmissibleFor (y<->x) p5
test86  = TestCase  $ assertBool "test86" $       isAdmissibleFor (y<->x) p6
test87  = TestCase  $ assertBool "test87" $       isAdmissibleFor (y<->x) p7
test88  = TestCase  $ assertBool "test88" $       isAdmissibleFor (y<->x) p8
...
test93  = TestCase  $ assertBool "test93" $       isAdmissibleFor (z<->x) p5
test94  = TestCase  $ assertBool "test94" $       isAdmissibleFor (z<->x) p6
test95  = TestCase  $ assertBool "test95" $       isAdmissibleFor (z<->x) p7
test96  = TestCase  $ assertBool "test96" $       isAdmissibleFor (z<->x) p8

test97  = TestCase  $ assertBool "test97" $ not $ isAdmissibleFor (x<->y) p1
test98  = TestCase  $ assertBool "test98" $ not $ isAdmissibleFor (x<->y) p2
test99  = TestCase  $ assertBool "test99" $ not $ isAdmissibleFor (x<->y) p3
test100 = TestCase  $ assertBool "test100"$       isAdmissibleFor (x<->y) p4
test101 = TestCase  $ assertBool "test101"$       isAdmissibleFor (x<->y) p5
test102 = TestCase  $ assertBool "test102"$       isAdmissibleFor (x<->y) p6
test103 = TestCase  $ assertBool "test103"$       isAdmissibleFor (x<->y) p7
test104 = TestCase  $ assertBool "test104"$       isAdmissibleFor (x<->y) p8
...
test113 = TestCase  $ assertBool "test113"$       isAdmissibleFor (z<->y) p1
test114 = TestCase  $ assertBool "test114"$ not $ isAdmissibleFor (z<->y) p2
test115 = TestCase  $ assertBool "test115"$ not $ isAdmissibleFor (z<->y) p3
test116 = TestCase  $ assertBool "test116"$       isAdmissibleFor (z<->y) p4
test117 = TestCase  $ assertBool "test117"$       isAdmissibleFor (z<->y) p5
test118 = TestCase  $ assertBool "test118"$       isAdmissibleFor (z<->y) p6
test119 = TestCase  $ assertBool "test119"$       isAdmissibleFor (z<->y) p7
test120 = TestCase  $ assertBool "test120"$       isAdmissibleFor (z<->y) p8
...
test125 = TestCase  $ assertBool "test125"$       isAdmissibleFor (x<->z) p5
test126 = TestCase  $ assertBool "test126"$       isAdmissibleFor (x<->z) p6
test127 = TestCase  $ assertBool "test127"$       isAdmissibleFor (x<->z) p7
test128 = TestCase  $ assertBool "test128"$       isAdmissibleFor (x<->z) p8

test129 = TestCase  $ assertBool "test129"$       isAdmissibleFor (y<->z) p1
test130 = TestCase  $ assertBool "test130"$ not $ isAdmissibleFor (y<->z) p2
test131 = TestCase  $ assertBool "test131"$ not $ isAdmissibleFor (y<->z) p3
test132 = TestCase  $ assertBool "test132"$       isAdmissibleFor (y<->z) p4
test133 = TestCase  $ assertBool "test133"$       isAdmissibleFor (y<->z) p5
test134 = TestCase  $ assertBool "test134"$       isAdmissibleFor (y<->z) p6
test135 = TestCase  $ assertBool "test135"$       isAdmissibleFor (y<->z) p7
test136 = TestCase  $ assertBool "test136"$       isAdmissibleFor (y<->z) p8
