...
-- f2 x = f2 y, so f2 not valid for Lx.Ly.x or Lx.Ly.(x y), Lx.Ly.(x (x y))
...
test12  = TestCase  $ assertBool "test12" $ isValidFor f2 p4
test13  = TestCase  $ assertBool "test13" $ not $ isValidFor f2 p5
test14  = TestCase  $ assertBool "test14" $ isValidFor f2 p6
test15  = TestCase  $ assertBool "test15" $ not $ isValidFor f2 p7
test16  = TestCase  $ assertBool "test16" $ not $ isValidFor f2 p8

-- f3 x = f3 z but z is not a variable of the pi's 
...
test24  = TestCase  $ assertBool "test24" $ isValidFor f3 p8

-- f4 y = f3 z but z is not a variable of the pi's 
...
test32  = TestCase  $ assertBool "test32" $ isValidFor f4 p8

-- (y<-:x) x = (y<-:x) y, so y<-:x is not valid for Lx.Ly.x ...
...
test36  = TestCase  $ assertBool "test36" $ isValidFor (y<-:x) p4
test37  = TestCase  $ assertBool "test37" $ not $ isValidFor (y<-:x) p5
test38  = TestCase  $ assertBool "test38" $ isValidFor (y<-:x) p6
test39  = TestCase  $ assertBool "test39" $ not $ isValidFor (y<-:x) p7
test40  = TestCase  $ assertBool "test40" $ not $ isValidFor (y<-:x) p8

-- (z<-:x) is valid for all
...
test48  = TestCase  $ assertBool "test48" $ isValidFor (z<-:x) p8

-- (z<-:y) is valid for all
...
test56  = TestCase  $ assertBool "test56" $ isValidFor (z<-:y) p8
