...
-- Convert the input String to [PasswdEntry]
  entries = map read (lines inp)  -- lines :: String -> [String]
...
main = do
  ...
  content <- readFile (head args) -- readFile :: FilePath -> IO String
  let maps = inputToMaps content
  mainMenu maps

mainMenu maps@(uidmap, usermap) = do  -- destructuring
  putStr optionText
  hFlush stdout -- hFlush :: Handle -> IO ()  , stdout :: Handle
  sel <- getLine -- getLine :: IO String 
  ...
  lookupUserName = do
    ...
    case Map.lookup username usermap of
      ...
      Just x -> print x -- print :: Show a => a -> IO ()
  lookupUID = do
    putStrLn "UID: "
    uidstring <- getLine
  ...
  optionText = -- breaking a string across several lines -> extra '\' at the end
               -- and at the beginning of each lines
    "\npasswdmap options:\n\
    \\n\
