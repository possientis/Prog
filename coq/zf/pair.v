...
(* a belongs to {a,b} *)
Proposition pair_left : forall a b:set, belong a (pair a b).
Proof.
  intros a b. apply pair_is_pair. left. reflexivity.
Qed.

(* b belongs to {a,b} *)
Proposition pair_right : forall a b:set, belong b (pair a b).
Proof.
  intros a b. apply pair_is_pair. right. reflexivity.
Qed.

(* x in {a,b} -> x = a \/ x = b *)
Proposition pair_elim : forall x a b:set, belong x (pair a b) -> x = a \/ x = b.
Proof.
