...
(* for the sequent G :- p. However, this assumption is not enough to create an  *)
(* explicit proof term, as this will depend on the position of p in G           *)
Lemma inCtx : forall (v:Type) (G:Ctx v) (p:P v),
  p :: G -> exists (e:G :- p), True.
Proof.
  ...
  (* We need to show the implication p :: G -> exists (e: G :- p), True *)
  assert (p :: G -> exists (e : G :- p), True) as A. 2: apply A.
  ...
  induction G as [|q G' IH].
    ...
    - intros H. assert (p :: G';q) as A. apply H. clear A. 
      ...
      (* We need to show the existence of e : G';q :- p *)
      assert (exists (e: G';q :- p), True) as A. 2: apply A.
...
Lemma compose : forall (v:Type) (G H:Ctx v),
  (forall (p:P v), p :: G -> exists (e:H :- p), True) -> 
  forall (p:P v), 
    (exists (e:G :- p), True) -> 
    (exists (e:H :- p), True). 
Proof.
  ...
  intros GH.
  assert (forall (p:P v), p :: G -> exists (e:H :- p), True) as A. 
  apply GH. clear A. 
  ...
  assert (exists (e':H :- p), True) as A. 2: apply A.
  ...
  induction e as 
    ...
    - assert (exists (e':H;p :- q), True) as K.
...
Lemma monotone : forall (v:Type) (G H:Ctx v) (p:P v),
  G <= H -> (exists (e:G :- p), True) -> (exists (e':H :- p), True).
Proof.
  ...
  assert (forall (q:P v), q :: G -> exists (e:H :- q), True) as A. 2: apply A.
  ...
  assert (exists (e:H :- q), True) as A. 2: apply A.
