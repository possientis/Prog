...
(* ValidT: Adding a type variable declaration to a valid context is valid.      *)
(* ValidV: Adding a variable declaration with a well-formed type expression     *)
(* to a valid context yields a valid context.                                   *)
Inductive Valid (b v:Type) : Context -> Prop :=
| ValidO : Valid b v O
...
(* The type expression of a binding in a valid context is always well-formed.   *)
Lemma ValidIsType : forall (b v:Type) (eq:Eq v) (G:Context) (x:v) (Ty:T b),
    Valid G -> G :>> x ::: Ty -> G :> Ty.
Proof.
    ...
    - intros x' Ty' H3. remember (G ; t ::: * ) as G' eqn:E.
      ...
      intros G' t' H6 H7 y' Sy H8 H9. 
        + inversion H8.
        + inversion H8. subst. apply IsTypeTS. apply H7 with y'. assumption.
        + inversion H8.
    - intros x' Ty' H3. apply IsTypeFS. apply findVRev' in H3.
      destruct H3 as [[H3 G3]|H3].
        + subst. assumption.
