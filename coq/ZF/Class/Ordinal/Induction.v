...
Proof.
  ...
  assert (exists a, (On :\: A) a /\ (On :\: A) :/\: toClass a :~: :0:) as H5. {
    apply OrdClass.HasMinimal with On. 3: assumption.
    - apply OrdClass.IsOrdinal.
    - apply Class.Inter2.IsInclL. }
  destruct H5 as [a [[H5 H6] H7]]. assert (toClass a :<: On) as H8. {
    apply OrdClass.LessIsElem; try assumption. apply OrdClass.IsOrdinal. }
  assert (toClass a :<=: A) as H9. {
    ...
    apply OrdClass.WhenElem with (toClass a); assumption. }
