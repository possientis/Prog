...
Proof.
  ...
  assert (f!x = y) as H12. { apply (FunctionOn.Eval f a); assumption. }
  assert (g!x = z) as H13. { apply (FunctionOn.Eval g b); assumption. }
...
Proof.
  ...
  assert (Transitive (CRD.domain (Recursion F))) as H1. {
    ...
    assert (x :< a) as H6. {
      destruct H3 as [_ H3]. rewrite <- H3. apply Domain.Charac.
      exists y. assumption. }
  ...
  assert (CRD.domain (Recursion F) :<=: On) as H2. {
    ...
    assert (x :< a) as H6. {
      destruct H4 as [_ H4]. rewrite <- H4. apply Domain.Charac.
      exists y. assumption. }
    apply Ordinal.IsOrdinal with a; assumption. }
...
Proof.
  ...
  assert (Domain.domain f = c) as H4. {
  ...
  assert (Relation f) as H5. {
    apply Relation.ToClass. apply Relation.EquivCompat with (Recursion F).
    1: assumption. apply IsRelation. }
  assert  (Functional f) as H6. {
    apply Functional.FromClass. apply CFL.EquivCompat with (Recursion F).
    1: assumption. apply IsFunction. }
  assert (Function f) as H7. { split; assumption. }
  assert (FunctionOn.FunctionOn f c) as H8. { split; assumption. }
  ...
  assert (forall g a, K' g a -> FunctionOn.FunctionOn (f:|:a) a) as H12. {
    intros g a H12. apply (FunctionOn.Restrict f c a). 1: assumption.
  ...
  assert (forall g a, K' g a -> g = f:|:a) as H13. {
    ...
    apply FunctionOn.Equal with a a. 1: assumption.
    ...
    - intros x H17.
      ...
      rewrite H18. symmetry. apply (FunctionOn.Eval f c). 1: assumption.
      apply H10. exists g. exists a. split. 2: assumption.
      apply FunctionOn.Satisfies with a; assumption. }
  assert (forall b, b :< c -> f!b = F!(f:|:b)) as H14. {
    ...
    assert (f!b = g!b) as H21. {
      symmetry. apply (FunctionOn.Eval g a); assumption. }
  ...
  assert (Function g) as H17. {
    ...
    - exfalso. apply Single.Charac in H18. apply OrdPair.Equal in H18.
      ...
      assert (c :< Domain.domain f) as H19. {
        apply Domain.Charac. exists y. assumption. }
  ...
  assert (FunctionOn.FunctionOn g (succ c)) as H18. {
  ...
  assert (g:|:c = f) as H19. {
    apply FunctionOn.Equal with c c. 2: assumption.
    - apply FunctionOn.Restrict with (succ c). 1: assumption. apply Succ.IsIncl.
    - reflexivity.
    - intros x H19.
      assert ((g:|:c)!x = g!x) as H20. {
        apply Restrict.Eval. 2: assumption. destruct H18 as [[_ H18] _].
        assumption. }
      rewrite H20. apply (FunctionOn.Eval g (succ c)). 1: assumption.
  ...
  assert (forall b, b :< succ c -> g!b = F!(g:|:b)) as H20. {
    ...
    - apply Single.Charac in H20.
      assert (g!b = F!f) as H21. {
        apply (FunctionOn.Eval g (succ c)). 1: assumption.
  ...
  assert (c :< Domain.domain f) as H23. {
...
Proposition Restrict : forall (F:Class) (a:U), On a ->
  FunctionOn.FunctionOn ((Recursion F) :|: a) a.
Proof.
  intros F a H1. split.
...
Proof.
  ...
  assert (b :< a) as H7. {
    destruct H5 as [_ H5]. rewrite <- H5. apply Domain.Charac.
    exists (Recursion F)!b. assumption. }
  assert ((Recursion F)!b = f!b) as H8. {
    symmetry. apply (FunctionOn.Eval f a); assumption. }
...
Proof.
  ...
  - split. 1: apply Equiv.Refl. apply Induction.Induction.
    intros a H3 H4.
    assert (Domain.domain (G:|:a) = a) as H6. {
    ...
    assert (Domain.domain ((Recursion F) :|: a) = a) as H7. {
