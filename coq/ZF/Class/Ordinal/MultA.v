...
(* The function class x -> x * a. Needed to define ordinal exponentiation.      *)
Definition MultA (a:U) : Class := fun x => exists b, x = :(b,b :*: a):.

Proposition Charac2 : forall (a x y:U), MultA a :(x,y): <-> y = x :*: a.
Proof.
...
Proposition DomainOf : forall (a b:U), domain (MultA a) b.
Proof.
  intros a b. exists (b :*: a). exists b. reflexivity.
Qed.

Proposition IsFunctional : forall (a:U), Functional (MultA a).
Proof.
  intros a x y z H1 H2. apply Charac2 in H1. apply Charac2 in H2.
  subst. reflexivity.
Qed.

Proposition Eval : forall (a b:U), (MultA a)!b = b :*: a.
Proof.
