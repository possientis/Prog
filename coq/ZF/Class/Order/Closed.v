...
(* The i-th unary image used in the finite closure construction.                *)
Definition unaryImage (R:Class) (i x:U) : U := truncate R$i:[toClass x]:.

(* The i-th binary image used in the finite closure construction.               *)
Definition binaryImage (S:Class) (i x:U) : U := truncate S$i:[toClass (x:x:x)]:.

(* The union of the finitely many unary images at one stage.                    *)
Definition unaryUnion (R:Class) (p x:U) : U :=
  :\/:_{p} (From.from (fun i => unaryImage R i x)).

(* The union of the finitely many binary images at one stage.                   *)
Definition binaryUnion (S:Class) (q x:U) : U :=
  :\/:_{q} (From.from (fun i => binaryImage S i x)).

(* One stage of closing under all listed operations.                            *)
Definition step (R S:Class) (p q x:U) : U :=
  x :\/: unaryUnion R p x :\/: binaryUnion S q x.

(* The omega-sequence obtained by iterating the closure step.                   *)
Definition seq (R S:Class) (p q a:U) : U :=
  recursion (From.from (step R S p q)) a.

(* The closure hull obtained by taking the union of all finite stages.          *)
Definition hull (R S:Class) (p q a:U) : U := :\/:_{:N} (seq R S p q a).

(* The closure sequence starts at the original set.                             *)
Proposition WhenZero : forall (R S:Class) (p q a:U),
  (seq R S p q a)!:0: = a.
Proof.
  (* Proof by Hermes + gpt 5.5                                                  *)
  intros R S p q a. unfold seq. rewrite RecursionNOfClass.WhenZero. reflexivity.
Qed.

(* The closure sequence advances by one closure step.                           *)
Proposition WhenSucc : forall (R S:Class) (p q a n:U), n :< :N ->
  (seq R S p q a)!(succ n) = step R S p q ((seq R S p q a)!n).
Proof.
  (* Proof by Hermes + gpt 5.5                                                  *)
  intros R S p q a n H1. unfold seq at 1.
  rewrite RecursionNOfClass.WhenSucc. 2: assumption.
  rewrite From.Eval. reflexivity.
Qed.

(* Every finite stage is included in the closure hull.                          *)
Proposition IsIncl : forall (R S:Class) (p q a n:U), n :< :N ->
  (seq R S p q a)!n :<=: hull R S p q a.
Proof.
  (* Proof by Hermes + gpt 5.5                                                  *)
  intros R S p q a n H1 x H2.
  apply UnionGen.Charac. exists n. split; assumption.
Qed.

(* The closure sequence is increasing.                                          *)
Proposition IsMono : forall (R S:Class) (p q a n m:U), n :< :N -> m :< :N ->
  n :<=: m -> (seq R S p q a)!n :<=: (seq R S p q a)!m.
Proof.
  (* Proof by Hermes + gpt 5.5                                                  *)
  intros R S p q a n m H1. revert m. apply Omega.Induction'. 1: assumption.
  - apply Incl.Refl.
  - intros m H2 H3 H4.
    apply Incl.Tran with ((seq R S p q a)!m). 1: assumption.
    rewrite WhenSucc. 2: assumption.
    apply Union2.IsInclL.
Qed.

(* The explicit closure hull has the expected closure properties.               *)
Proposition Hull : forall (R S A:Class) (p q a:U),
  p :< :N                                                                   ->
  q :< :N                                                                   ->
  toClass a :<=: A                                                          ->
  ...
  (forall i x, i :< q -> toClass x :<=: A -> Small S$i:[toClass (x:x:x)]:)  ->
    a :<=: hull R S p q a                                                   /\
    toClass (hull R S p q a) :<=: A                                         /\
    (forall i, i :< p -> Closed  R$i (toClass (hull R S p q a)))            /\
    (forall i, i :< q -> Closed2 S$i (toClass (hull R S p q a))).
Proof.
  (* Proof by Hermes + gpt 5.5                                                  *)
  intros R S A p q a H1 H2 H3 H4 H5 H6 H7.
  remember (seq R S p q a) as f eqn:H8.
  remember (hull R S p q a) as b eqn:H9.
  assert (:0: :< :N) as G0. { apply Omega.HasZero. }
  assert (domain f = :N) as G1. {
    rewrite H8. apply RecursionNOfClass.IsFunctionOn. }
  assert (Functional f) as G2. {
    rewrite H8. apply RecursionNOfClass.IsFunctionOn. }
  assert (forall x, x :< b <-> exists n, n :< :N /\ x :< f!n) as G3. {
    rewrite H9, H8. apply UnionGen.Charac. }
  assert (forall n, n :< :N -> f!n :<=: b) as G4. {
    intros n G4 x G5. apply G3. exists n. split; assumption. }
  assert (a = f!:0:) as G5. { rewrite H8. symmetry. apply WhenZero. }
  assert (forall n, n :< :N ->
    f!(succ n) = f!n :\/: unaryUnion R p f!n :\/: binaryUnion S q f!n) as G6. {
    intros n G6. rewrite H8. rewrite WhenSucc. 2: assumption. reflexivity. }
  assert (forall x, toClass x :<=: A -> toClass (unaryUnion R p x) :<=: A) as G7. {
    intros c G7. apply UnionGenOfClass.WhenClassBounded.
    intros i G8. rewrite From.Eval.
    apply Truncate.IsIncl. apply CIN.Tran with R$i:[A]:.
    - apply CRI.InclCompatR. assumption.
    - apply H4. assumption. }
  assert (forall x, toClass x :<=: A -> toClass (binaryUnion S q x) :<=: A) as G8. {
    intros x G8. apply UnionGenOfClass.WhenClassBounded.
    intros i G9. rewrite From.Eval.
  ...
  assert (forall n m, n :< :N -> m :< :N -> n :<=: m -> f!n :<=: f!m) as G10. {
    ...
    - intros m G11 G12 G13. apply Incl.Tran with f!m. 1: assumption.
      apply G9. assumption. }
  assert (forall n, n :< :N ->
    unaryUnion R p f!n :\/: binaryUnion S q f!n :<=: f!(succ n)) as H16. {
    intros n H16. rewrite G6. 2: assumption. apply Union2.IsInclR. }
  assert (forall n, n :< :N -> unaryUnion R p f!n :<=: f!(succ n)) as H17. {
    intros n H17.
    apply Incl.Tran with (unaryUnion R p f!n :\/: binaryUnion S q f!n).
    - apply Union2.IsInclL.
    - apply H16. assumption. }
  assert (forall n, n :< :N -> binaryUnion S q f!n :<=: f!(succ n)) as H18. {
    intros n H18.
    apply Incl.Tran with (unaryUnion R p f!n :\/: binaryUnion S q f!n).
  ...
  assert (forall i, i :< p -> Closed R$i (toClass b)) as H22. {
    ...
    apply Union2.Charac3. right. left.
    apply UnionGenOfClass.Charac. exists i. split. 1: assumption.
    rewrite From.Eval. apply Truncate.Charac. split.
...
Proof.
  (* Proof by Hermes + gpt 5.5                                                  *)
  intros R S A p q a H1 H2 H3 H4 H5 H6 H7. exists (hull R S p q a).
  apply Hull; assumption.
Qed.
