...
Record Category : Type := category
    ...
    ; proof_idl     : forall (a b:Obj) (f:Hom a b), 
        equal (eqHom a b) (compose _ _ _ (id b) f) f
    ; proof_idr     : forall (a b:Obj) (f:Hom a b),
        equal (eqHom a b) (compose _ _ _ f (id a)) f 
    ; proof_assoc   : forall (a b c d:Obj)(f:Hom a b)(g:Hom b c)(h:Hom c d),
        equal (eqHom a d) (compose _ _ _ h (compose _ _ _ g f))
                          (compose _ _ _ (compose _ _ _ h g) f)
    ; proof_compat  : forall (a b c:Obj) (f f':Hom a b) (g g':Hom b c),
        equal (eqHom a b) f f' -> equal (eqHom b c) g g' -> 
        equal (eqHom a c) (compose _ _ _ g f) (compose _ _ _ g' f')
    }
    .
...
Arguments proof_assoc {c} {a} {b} {c0} _ _ _ _.

Notation "g @ f" := (compose g f) (at level 60, right associativity). 

Notation "f == g" := (equal eqHom f g) (at level 90, no associativity).
