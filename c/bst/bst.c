// BST.c
// should be included in header file
//#include "BST.h"
...
void BST<Key,T>::insert(Key key, T* value_p){
...
T* BST<Key,T>::min(Key& key) const{

  BSTNode<Key,T> *temp = minNode(d_top_p);  // node with minimal key
...
T* BST<Key,T>::max(Key& key) const{

  BSTNode<Key,T> *temp = maxNode(d_top_p);  // node with maximal key
...
T* BST<Key,T>::find(Key key) const{

  BSTNode<Key,T> *temp = findNode(d_top_p, key, d_comp_p);  // node with given key
...
T* BST<Key,T>::succ(Key key, Key& succ) const{

  BSTNode<Key,T> *temp = succNode(d_top_p, key, d_comp_p);  // node with succ key
...
T* BST<Key,T>::pred(Key key, Key& pred) const{

  BSTNode<Key,T> *temp = predNode(d_top_p, key, d_comp_p);  // node with pred key
...
static BSTNode<Key,T> *insertNode(
  ...
  BSTNode<Key,T> *node_p, Key key, T *value_p, bool (*comp_p)(Key,Key)){
  ...
  if(comp_p(key,node_p->key())){  // key is to the left
  ...
  if(comp_p(node_p->key(),key)){  // key is to the right
...
static BSTNode<Key,T> *deleteNode(
    BSTNode<Key,T> *node_p, Key key, bool (*comp_p)(Key,Key)){
  ...
  if(comp_p(node_p->key(),key)){  // key is to thr right
...
static BSTNode<Key,T> *minNode(BSTNode<Key,T> *node_p){
...
static BSTNode<Key,T> *maxNode(BSTNode<Key,T> *node_p){
...
static BSTNode<Key,T> *succNode(
  ...
  if(comp_p(key,node_p->key())){ // key is to the left
    ...
    BSTNode<Key,T> *temp = succNode(node_p->left(),key,comp_p);  // looking left
  ...
  if(comp_p(node_p->key(),key)){ // key is to the right, if successor exists ...
  ...
  if (node_p->right() == nullptr){  // right child does not exist
    ...
    if(comp_p(key,parent_p->key())){  // parent key greater, it is successor
...
static BSTNode<Key,T> *predNode(
  ...
  if(comp_p(node_p->key(),key)){ // key is to the right
    ...
    BSTNode<Key,T> *temp = predNode(node_p->right(),key,comp_p);  // looking right
  ...
  if(comp_p(key,node_p->key())){ // key is to the left, if predecessor exists ..
  ...
  if (node_p->left() == nullptr){  // left child does not exist
    ...
    if(comp_p(parent_p->key(), key)){  // parent key smaller, it is predecessor
...
static bool checkInvariantNode(
    BSTNode<Key,T> *node_p, bool (*comp_p)(Key,Key)){
...
static bool checkParentNode(BSTNode<Key,T> *node_p){
