...
(define (unit-test)
  ...
  (let ((env ((global-env 'extended) 'var #f)))    ; extended env with var = #f
    ...
    (let ((result (lazy-eval '(set! var #\a) env))) 
      (assert-equals result unspecified-value "assignment.4")
      (test-expression 'var #\a "assignment.5" env))
    ; analyzed assignment
  ...
  (let ((env ((global-env 'extended) 'var #f)))
    ...
    (let ((result (lazy-eval '(define (f) 0.3) env)))
      (assert-equals result unspecified-value "definiton.58")
    ...
    ; lazy definition: function with a single argument 
    (let ((result (lazy-eval '(define (f x) (* x x x)) env)))
      (assert-equals result unspecified-value "definiton.93")
    ...
    ; lazy definition: function with two arguments 
    (let ((result (lazy-eval '(define (f x y) (* x y)) env)))
      (assert-equals result unspecified-value "definiton.128")
    ...
    (let ((result (lazy-eval '(define (f . args) args) env)))
      (assert-equals result unspecified-value "definiton.173")
    ...
    (let ((result (lazy-eval '(define (f x . args) (cons x args)) env)))
      (assert-equals result unspecified-value "definiton.260")
