...
// TODO: figure out how to implement 128 bits signed integers

typedef struct { uint64_t low, 
                 uint64_t  high; 
} int128_t;

extern int128_t *test_imul_64bits(int64_t, int64_t, int reg);
...
int main()
{
  ...
  int64_t p32 = 4000000063L;     // prime which fits in 32 bits
  ...
  int64_t _2_64_mod_p32 = ((1UL << 32) % p32)       // 32 bits
      ...
      for(x0 = 0; x0 < 4294967296UL; x0 += 90000083) {
        for(y1 = 0; y1 < 4294967296UL; y1 += 90000083) {
          for(y0 = 0; y0 < 4294967296UL; y0 += 90000083) {
            ...
            int64_t x64 = x1*4294967296UL + x0; // unsigned result assigned to signed var
            int64_t y64 = y1*4294967296UL + y0;

            // obtaing result from assembly instruction 'imul <reg>'
            ...
            int128_t x128 = *test_mul_64bits(x64, y64, i);
         

            int64_t x32  = x64 % p32;         // 32 bits 'projection'
            int64_t y32  = y64 % p32;         // 32 bits 'projection'

            uint64_t h32  = x128.high % p32;   // proj of high order quad
            uint64_t l32  = x128.low  % p32;  // proj of low order quad

            // Denoting h64 = x128.high (signed) and l64 = x128.low (which
            // is not signed), h64 and l64 are respectively the high (signed)
            // and low (unsigned) order quad words of the 128 bits quantity 
            // (unsigned) returned by the assembly function which is meant 
            // to compute the product of x64 and y64. Hence the equality:
            //
            //   (*) x64 * y64 = h64 * 2^64 + l64     (if h64 >= 0)
            //   (*) x64 * y64 = h64 * 2^64 - l64     (if h64 <  0)
            //
