...
// yet it is claim that the following works, hence a paradox
// This answer to the paradox is: in general (arbitrary 
// equivalence relation RA on super class), you cannot redefine 
// equality on the subclass and hope to obtain an equivalence relation.
// Of course we can always keep the equivalent pairs of (A\B)x(A\B)
// (A\B)xB and B\(A\B) and replace those of BxB by the new definition,
// but the point is that this new subset of AxA is not (in general), an
// equivalence relation on A. Then you can take the equivalence relation
// on A generated by this set of pairs, but then your code needs to be
// changed in both classes so as to return the right boolean value,
// reflecting the fact it is now implementating a 'generated' equivalence
// relation. So the point is, if you start overriding equality in B,
// you will need to change the code in A, you cannot leave the code of A
// unchanged. This is the general case.
//
// However, if you start with a relation on A whose trace is empty on
// Bx(A\B) and (A\B)xB, then things are different. You can freely redefine
// the equivalent pairs on BxB with a new equivalence relation, and the 
// resulting set of pairs is still an equivalence relation. There is no
// need to revisit the code of class A.
// 
class Point5(val x: Int, val y: Int) {
  ...
  def isComparable(other: Any) = other.isInstanceOf[Point5] // instance, type
}
...
class ColoredPoint2(x: Int, y: Int, val color: Color.Value) 
  ...
  override def isComparable(other: Any) = other.isInstanceOf[ColoredPoint2]
}
