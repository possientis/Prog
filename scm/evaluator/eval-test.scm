...
(define (eval-test)
  ...
  (let ((x (eval ''(list cons 3 "abc" #\a #t))))
  ...
  (let ((saved-value (eval 'modulo)))
    ...
    (let ((x (eval '(set! modulo 'any-value))))
      (if (not (equal? (eval 'modulo) 'any-value))
  ...
  (let ((x (eval '(define var1 12))))
    (if (not (equal? (eval 'var1) 12))
      (display "eval: unit test 5.0 failing\n")))
  (let ((x (eval '(define var2 0.3))))
    (if (not (equal? (eval 'var2) 0.3))
      (display "eval: unit test 5.1 failing\n")))
  (let ((x (eval '(define var3 "hello"))))
    (if (not (equal? (eval 'var3) "hello"))
      (display "eval: unit test 5.2 failing\n")))
  (let ((x (eval '(define var4 #\a))))
    (if (not (equal? (eval 'var4) #\a))
      (display "eval: unit test 5.3 failing\n")))
  (let ((x (eval '(define var5 #t))))
    (if (not (equal? (eval 'var5) #t))
      (display "eval: unit test 5.4 failing\n")))
  (let ((x (eval '(define var6 #f))))
    (if (not (equal? (eval 'var6) #f))
  ...
  (let ((x (eval '(define (f) 12))))
    (if (not (equal? (eval '(f)) 12))
      (display "eval: unit test 5.6 failing\n"))
    ((global-env 'delete!) 'f))
  (let ((x (eval '(define (f x) (* x x)))))
    (if (not (equal? (eval '(f 5)) 25))
      (display "eval: unit test 5.7 failing\n"))
    ((global-env 'delete!) 'f))
  (let ((x (eval '(define (f x y) (+ x y)))))
    (if (not (equal? (eval '(f 3 4)) 7))
 ...
 ;
  ...
  (let ((plus5 (eval '(lambda (x) (+ x y)) ((global-env 'extended)'(y)'(5)))))
    ...
    ((global-env 'delete!) 'f))
  (let ((x (eval '((lambda () 45)))))
    (if (not (equal? x 45)) (display "eval: unit test 8.4 failing\n")))
  (let ((x (eval '((lambda (x) (+ x 7)) 5))))
    (if (not (equal? x 12)) (display "eval: unit test 8.5 failing\n")))
  (let ((x (eval '(let ((x 5)) ((lambda (u v) (+ u v)) x 6)))))
    (if (not (equal? x 11)) (display "eval: unit test 8.6 failing\n")))
  (let ((x (eval '(lambda arg (apply + arg)))))
    ((global-env 'define!) 'f x)
    (let ((y (eval '(f 1 2 3 4 5))))
      (if (not (equal? y 15)) (display "eval: unit test 8.6.1 failing\n")))
    ((global-env 'delete!) 'f))
  (let ((x (eval '(lambda (x y z . t) (+ x y z (apply + t))))))
    ((global-env 'define!) 'f x)
    (let ((y (eval '(f 1 2 3 4 5))))
      (if (not (equal? y 15)) (display "eval: unit test 8.6.2 failing\n")))
    (let ((y (eval '(f 1 2 3 4))))
      (if (not (equal? y 10)) (display "eval: unit test 8.6.3 failing\n")))
    (let ((y (eval '(f 1 2 3))))
      (if (not (equal? y 6)) (display "eval: unit test 8.6.4 failing\n")))
    ((global-env 'delete!) 'f))
  ...
  (let ((x ((analyze '(lambda () (+ 3 5))) global-env)))
    ...
    (let ((y ((analyze '(f)) global-env)))
      (if (not (equal? y 8)) (display "eval: unit test 8.7 failing\n")))
    ((global-env 'delete!) 'f))
  (let ((x ((analyze '(lambda (x) (* 3 x))) global-env)))
    ((global-env 'define!) 'f x)
    (let ((y ((analyze '(f 5)) global-env)))
      (if (not (equal? y 15)) (display "eval: unit test 8.8 failing\n")))
    ((global-env 'delete!) 'f))
  (let ((x ((analyze '(lambda (x y) (+ x y))) global-env)))
    ((global-env 'define!) 'f x)
    (let ((y ((analyze '(f 3 4)) global-env)))
      (if (not (equal? y 7)) (display "eval: unit test 8.9 failing\n")))
    ((global-env 'delete!) 'f))
  (let ((plus5 ((analyze '(lambda (x) (+ x y))) ((global-env 'extended)'(y)'(5)))))
    ((global-env 'define!) 'f plus5)
    (let ((y ((analyze '(f 6)) global-env)))
      (if (not (equal? y 11)) (display "eval: unit test 8.10 failing\n")))
    ((global-env 'delete!) 'f))
  (let ((x ((analyze '((lambda () 45))) global-env)))
    (if (not (equal? x 45)) (display "eval: unit test 8.11 failing\n")))
  (let ((x ((analyze '((lambda (x) (+ x 7)) 5)) global-env)))
    (if (not (equal? x 12)) (display "eval: unit test 8.12 failing\n")))
  (let ((x ((analyze '(let ((x 5)) ((lambda (u v) (+ u v)) x 6))) global-env)))
    (if (not (equal? x 11)) (display "eval: unit test 8.13 failing\n")))
  (let ((x ((analyze '(lambda arg (apply + arg))) global-env)))
    ((global-env 'define!) 'f x)
    (let ((y ((analyze '(f 1 2 3 4 5)) global-env)))
      (if (not (equal? y 15)) (display "eval: unit test 8.14 failing\n")))
    ((global-env 'delete!) 'f))
  (let ((x ((analyze '(lambda (x y z . t) (+ x y z (apply + t)))) global-env)))
    ((global-env 'define!) 'f x)
    (let ((y ((analyze '(f 1 2 3 4 5)) global-env)))
      (if (not (equal? y 15)) (display "eval: unit test 8.15 failing\n")))
    (let ((y ((analyze '(f 1 2 3 4)) global-env)))
      (if (not (equal? y 10)) (display "eval: unit test 8.16 failing\n")))
    (let ((y ((analyze '(f 1 2 3)) global-env)))
      (if (not (equal? y 6)) (display "eval: unit test 8.17 failing\n")))
    ((global-env 'delete!) 'f))
  ;
  ; begin
  ;
  ...
  (let ((x (eval '(letrec 
                    ...
                    (loop 5)))))
    (if (not (equal? x 120)) (display "eval: unit test 15.2 failing\n")))
