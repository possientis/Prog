...
\begin{proof}
    ...
    \times\Cat$, $b=(b_{1},b_{2})\in\Cop\times\Cat$ and $f=(f_{1},f_{2}):
    (a_{1},a_{2})\to(b_{1},b_{2})\ @\ \Cop\times\Cat$. Since $F_{0}(a)=
    \Cat(a_{1},a_{2})$ and $F_{0}(b)=\Cat(b_{1},b_{2})$, we need to check 
    that $F_{1}(f_{1},f_{2}):\Cat(a_{1},a_{2})\to \Cat(b_{1},b_{2})$.
    However, we have already established that fact in part $(2)$ of this 
    proof, provided we show that $f_{1}:b_{1}\to a_{1}\ @\ \Cat$ and
    $f_{2}:a_{2}\to b_{2}\ @\ \Cat$. So we need to check that
    $(f_{1},f_{2})\in\Cat(b_{1},a_{1})\times\Cat(a_{2},b_{2})$ which
    goes as follows:
