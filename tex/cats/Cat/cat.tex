...
\begin{notation}\label{Cat:notation:domain}
    Let \Cat\ be a category. The domain of $f\in\arr\ \Cat$ is denoted 
    $\dom(f)$.
\end{notation}


\begin{notation}\label{Cat:notation:codomain}
    Let \Cat\ be a category. The codomain of $f\in\arr\ \Cat$ is denoted 
    $\cod(f)$.
\end{notation}

\noindent
{\bf Remark}: Notations~(\ref{Cat:notation:domain}) 
and~(\ref{Cat:notation:codomain}) are potentially ambiguous as a mathematical 
object $f$ could in principle be an arrow in several categories, and the
designations $\dom(f)$ and $\cod(f)$ do not specfify which category is
being referred to.
