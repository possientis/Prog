...
are the left-hand-side and right-hand-side functors associated with $(F,G)$. 
    \[
        \begin{tikzcd}
...
where $F^{*}$ and $G^{*}$
are the left-and-side and right-hand-side functors of $(F,G)$. 
    \[
        \begin{tikzcd}
...
\begin{proof}
    ...
    $(\epsilon F \circ F\eta)(c)=\iota_{F}(c)$:
        \begin{eqnarray*}(\epsilon F \circ F\eta)(c)
            &=&\epsilon_{F(c)}\circ F(\eta_{c})
            \ \leftarrow\ \mbox{done before}\\
            \mbox{set $d=F(c)$}\ \to\ 
            &=&\epsilon_{d}\circ F(\eta_{c})\\
            \mbox{$\eta_{c}:c\to G(d)$,\ 
            eqn.~(\ref{Adj:eqn:beta:G*:F*:epsilon:2})}\ \to\ 
            &=&\beta(c,d)(\eta_{c})\\
            \mbox{eqn.~(\ref{Adj:eqn:alpha:F*:G*:eta:1})}\ \to\ 
            &=&\beta(c,d)(\ \alpha(c,d)(\,\id(d)\,) \ )\\
            \mbox{$\circ$ in \Set}\ \to\ 
            &=&(\ \beta(c,d)\,\circ\,\alpha(c,d)\ )(\,\id(d)\,)\\
            \mbox{def.~(\ref{Nat:def:composition})}\ \to\ 
            &=&(\beta\circ\alpha)(c,d)(\,\id(d)\,)\\
            \mbox{$\beta\circ\alpha=\iota_{F^{*}}$}\ \to\ 
            &=&\iota_{F^{*}}(c,d)(\,\id(d)\,)\\
            \mbox{def.~(\ref{Nat:def:identity})}\ \to\ 
            &=&\id(\,F^{*}(c,d)\,)(\,\id(d)\,)\\
            \mbox{prop.~(\ref{Adj:prop:lhs:object})}\ \to\ 
